...
{
  private $sock;
  public $host = "192.168.0.13";
  public $port = "502";
  public $client = "";
  public $client_port = "502";
  public $status;
  public $timeout_sec = 60; // Timeout 30 sec
  ...
  function error_write($error_msg)
  {
    date_default_timezone_set('Asia/Kolkata');
    $myfile = fopen("logFile.txt", "a") or die("Unable to open file!");
    fwrite($myfile, date("Y-m-d h:i:sa") . $error_msg . "\n");
    fclose($myfile);
  }
  ...
  public function connect()
  {
    ...
    if ($result === false) {
      $this->status .= false;
      $this->error_write("NotConnected");
      // $this -> error_write("socket_connect() failed");
      // throw new Exception("socket_connect() failed.</br>Reason: ($result)".
      //     socket_strerror(socket_last_error($this->sock)));

    } else {
      $this->status .= true;
      $this->error_write("Connected");
      return true;
    }
  }
   ...
   * @return string
   */
  public function readMultipleRegistersPacketBuilder($unitId, $reference, $quantity)
  {
    $dataLen = 0;
    // build data section
    $buffer1 = "";
